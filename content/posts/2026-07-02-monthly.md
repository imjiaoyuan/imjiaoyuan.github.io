...
- 开发了[jsrc](https://github.com/imjiaoyuan/jsrc)和[jkey](https://github.com/imjiaoyuan/jkey)，这两个 pip 包，之前说过了不再赘述，不过最近打包了 aur 包[jsrc](https://aur.archlinux.org/packages/jsrc)和[jkey](https://aur.archlinux.org/packages/jkey)，因为我习惯于使用统一的包管理器去管理软件包，使用 uv tool 感觉不爽，用 aur 安装可以方便在 ArchLinux 上的使用。
