...
评论基于 Giscus，一个利用 GitHub Discussions 驱动的评论组件。在 `config.py` 里配置好 repo、repo_id、category 和 category_id，每篇文章底部自动加载讨论区：
...
通过 `data-mapping="title"` 以文章标题作为映射，同一标题的文章会自动匹配到已有讨论。主题切换时，通过 `localStorage` 的 `site-theme` 和 `site:theme-change` 自定义事件同步 Giscus 的亮暗主题：

```javascript
window.addEventListener('site:theme-change', function(e) {
    setGiscusTheme(e.detail.theme);
});
document.addEventListener('giscus:ready', function() {
    setGiscusTheme(stored);
});
```

不用额外注册任何第三方评论服务，仓库的 Discussions 就是评论数据库。
