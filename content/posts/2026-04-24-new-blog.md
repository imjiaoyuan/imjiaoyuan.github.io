...
整个博客就一个 GitHub 仓库，内容、源码、生成的静态文件都在里面。提交后 GitHub Actions 会自动执行构建，生成 HTML 然后部署到 Pages。整个过程全自动，不用手动敲命令，也不用登录什么后台。本地构建的时候可以使用 `run.py`：

```
python run.py -d            # 构建静态站点到 public/
python run.py -s            # 构建并启动开发服务器（热重载）
python run.py -n post-name  # 创建新文章
python run.py -f            # 格式化所有文章（pangu 间距、清理多余空行）
python run.py -u image.png  # 上传图片到 R2（自动转 WebP，输出短链 URL）
```
