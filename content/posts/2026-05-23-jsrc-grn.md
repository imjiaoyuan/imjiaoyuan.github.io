...
所有模块都打在这一个包里，装完就有全部功能，第三方依赖只有画图用的 matplotlib。如果想让启动快一点，还可以用环境变量只启用需要的模块：

```bash
export JSRC_MODULES=grn
```
...
- `-a, --all`：全视图模式，节点数不超过阈值时一眼摊开所有节点
- `-e, --expand`：点击展开模式，需要点击节点才展开邻居
- 两个开关都不给就是 auto：按节点数和阈值自动决定用哪种模式
...
- 节点颜色：当前的中心基因是红色，其余节点是蓝色，鼠标点选的节点会高亮成黄色
- 边的粗细表示调控强度（weight 值），不同轮次展开的边颜色不同
- 左上角面板显示节点计数和邻居列表，点击邻居可以跳转查看
...
- `-d, --dir`：服务目录，默认当前目录
- `-p, --port`：端口号，默认 8000
- `-a, --all`：全视图模式（节点数不超过阈值时自动展开）
- `-e, --expand`：点击展开模式
- `-t, --threshold`：全视图阈值，默认 300

## 性能优化

对于大规模网络（节点数明显超过默认阈值 300），建议使用点击展开模式：
