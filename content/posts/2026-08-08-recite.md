...
背单词的核心难题就是遗忘。今天背得再熟，过两周不看大概率忘光。所以 recite 的核心是间隔重复（spaced repetition），用的是 FSRS（Free Spaced Repetition Scheduler）算法——墨墨开源、Anki 新版本主推的那套调度器，在网页里通过 `ts-fsrs` 实现。

思路说起来不复杂：你打分越高，说明记得越牢，下次复习的间隔就拉得越长；打分低、甚至完全忘了的词，间隔就缩短，很快再来一遍。理想状态下，每个词总在它快要被你忘掉的那一刻重新出现，既不浪费时间去反复看熟词，也不至于让生词冷在一边。

FSRS 和传统的 SM-2 最大的区别是它真的在给"记忆"建模：每个词维护两个内部量，稳定度（stability，记忆大概能保持多久）和难度（difficulty，这个词对你来说有多难），每次打分更新这两个量，再由它们算出下次的复习日期。具体到代码，一次打分就是重建卡片、喂给调度器、把结果存回去：
...
四个按钮映射成 FSRS 的四个评级：重来是 1（Again），困难 3（Hard），良好 4（Good），简单 5（Easy）。稳定度涨得快，下一次就隔得远；答一次"重来"会被记成一次遗忘（lapse），难度抬高、稳定度回落，隔天就又见面。而且稳定度的增长不是简单乘一个系数，而是跟当前记忆强度和已经过去的间隔时间一起算出来的——前期间隔涨得慢、后面越涨越快，比较贴近真实的遗忘曲线。

设置里的"目标保留率"就是喂给 FSRS 的参数：宽松 80%、标准 90%、严格 95%，调得越高，算法就越倾向于把复习安排得密集一些。之前用 SM-2 存下来的进度也不会丢，下次打分时会自动迁移（老的 interval 当作稳定度，ease factor 反推难度），老备份导进来照样接着背。
...
额度是按天累计的，记在 `recite:daily` 里，所以补新词时要用 `limit - todayNew(listId)` 把今天已经背过的量扣掉——一天背几次都共用同一份额度。另外被标记为"已掌握"（known）的词不会再进入复习队列。
...
- **难词本**：复习和拼写里答错、答"重来"的词会自动收进来，从首页顶部的按钮单独过一遍，专治那些反复记不住的顽固词汇。
- **查词**：不在背诵流程里，就是随手搜一个词，看它的意思、音标、例句，相当于内置了个小词典。

每个词表还有一页词表浏览，能按"新词 / 待复习 / 已学 / 已掌握 / 难词"筛选，也可以多选之后批量标记为"已掌握"——那些你早就会的词直接划掉，别再占额度了。
...
这是我最在意的一点。recite 没有账号、没有后端、没有数据库，你所有的进度都存在自己浏览器的 localStorage 里，一共就五个 key：

```ts
const STATE_KEY = 'recite:state';    // 每个词的复习状态
const META_KEY = 'recite:meta';      // 全局设置
const ACT_KEY = 'recite:activity';   // 每日打卡记录
const USER_KEY = 'recite:userlists'; // 自己建的词表
const DAILY_KEY = 'recite:daily';    // 今日新词额度计数
```
...
既然数据都在本地，就得能搬走。设置里可以一键把全部进度导出成一个 JSON 文件，换设备、换浏览器时再导回去就行。设置也都在本地：发音可选美音或英音、每日新词数（20 到 200）、目标保留率（80%/90%/95%）、拼写倒计时、主题（跟随系统/亮/暗）、界面语言（自动/中文/英文）。
...
和我的博客是一个思路，不引一堆依赖，自己掌控所有逻辑，想咋改咋改，出了问题也知道去哪修。整个项目加起来也就两千八百来行 TypeScript，没有状态管理库、没有构建模板，一个 vite 直接打包成静态文件，往 GitHub Pages 上一扔就能跑。
