...
- **`src/config.py`** — The only file you edit to configure the site. Defines a `SITE` dict (title, domain, description, menu, etc.). The `description` field feeds `<meta>` tags and the Atom feed subtitle. Loaded at runtime via `importlib`.
